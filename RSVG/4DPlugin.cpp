...
void SVG_Convert_array(sLONG_PTR *pResult, PackagePtr pParams)
{
	...
	C_TEXT _uri;
#ifndef _WIN32
	NSString *url = Param7_Uri.copyUrlString();
	_uri.setUTF16String(url);
	[url release];
#else
	CUTF16String path;
	Param7_Uri.copyUTF16String(&path);
	...
	if(UrlCreateFromPath((const wchar_t *)path.c_str(), fileUrl, &urlLen, NULL) == S_OK)
		_uri.setUTF16String((const PA_Unichar*)fileUrl, urlLen);
#endif
	
	CUTF8String uri;
	_uri.copyUTF8String(&uri);
